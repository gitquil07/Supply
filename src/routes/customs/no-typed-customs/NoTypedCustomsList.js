...
const NoTypedCustomsList = ({match, getCustomsList}) => {
    const columns = [
        {
            name: "public_id",
            label: "Заказ номера",
    ...
    useEffect(() => {
        getCustomsList(moment(from).format("YYYY-MM-DD"), moment(to).format("YYYY-MM-DD"), "ждет льготы")
    },[from, to])
    ...
    return (
        <div className="map-wrapper">
            <PageTitleBar title={<IntlMessages id="sidebar.no-typed-customs" />} match={match} />
            <RctCollapsibleCard>
